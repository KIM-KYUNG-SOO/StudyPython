...
def setDataIntoDivtbl(conn,tup):
    ...
    cur.close()
    conn.commit() # DB에서 commit과 같음 #conn.rollback도 마찬가지, COMMIT 필수!
...
def deleteDivision(conn, division):
    ...
    query = '''DELETE FROM divtbl
                WHERE division = :1'''
    cur.execute(query, (division,)) # 값을 튜플로 바꾼형태
    # 데이터가 하나일때 튜플로 변경시에 반드시,를 넣어줘야한다.
    conn.commit()
...
if __name__ == '__main__':
    ...
    # # 6. divtbl에 임의 데이터 삭제
    print('책 장르정보 삭제')
    division = input('삭제할 장르코드 입력:').upper()
